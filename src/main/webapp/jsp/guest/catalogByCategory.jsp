			...
			<nav>
				<ul class="pagination">
					<c:if test="${currentPage != 1}">
						<li class="page-item">
							<a class="page-link" href="Controller?action=openCatalogByCategotyIdPage&recordsPerPage=${recordsPerPage}&currentPage=${currentPage-1}">
					...
					<c:forEach begin="1" end="${pagesQuantity}" var="i">
						<c:choose>
							...
							<c:otherwise>
								<li class="page-item">
									<a class="page-link" href="Controller?action=openCatalogByCategotyIdPage&recordsPerPage=${recordsPerPage}&currentPage=${i}">${i}</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
					...
					<c:if test="${currentPage lt pagesQuantity}">
						<li class="page-item">
							<a class="page-link" href="Controller?action=openCatalogByCategotyIdPage&recordsPerPage=${recordsPerPage}&recordsPerPage=${recordsPerPage}&currentPage=${currentPage+1}">
